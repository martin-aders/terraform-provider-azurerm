...
func resourceStorageShareDirectoryCreate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	var storageShareId *parse.StorageShareDataPlaneId
	var err error
	if v, ok := d.GetOk("storage_share_id"); ok && v.(string) != "" {
		storageShareId, err = parse.StorageShareDataPlaneID(v.(string), storageClient.StorageDomainSuffix)
		if err != nil {
			return err
		}
	} else {
		// TODO: this is needed until `share_name` / `storage_account_name` are removed in favor of `storage_account_id`
		storageShareId = pointer.To(parse.NewStorageShareDataPlaneId(d.Get("storage_account_name").(string), storageClient.StorageDomainSuffix, d.Get("share_name").(string)))
	}
	...
	if err != nil {
		return fmt.Errorf("retrieving Account %q for Directory %q (Share %q): %v", storageShareId.AccountName, directoryName, storageShareId.Name, err)
	}
	if account == nil {
	...
	id := directories.NewDirectoryID(*accountId, storageShareId.Name, directoryName)
	...
	existing, err := client.Get(ctx, storageShareId.Name, directoryName)
	if err != nil {
	...
	if _, err = client.Create(ctx, storageShareId.Name, directoryName, input); err != nil {
...
func resourceStorageShareDirectoryRead(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	storageShareId := parse.NewStorageShareDataPlaneId(id.AccountId.AccountName, storageClient.StorageDomainSuffix, id.ShareName)
